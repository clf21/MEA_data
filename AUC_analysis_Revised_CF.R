...
set_output <- "all_hill_plots" # This can be set to "replicates", "dr_plot", "all_dr_plots", "hill_plot", "all_hill_plots", "ec50_table","toxcast_table", or "all_toxcast_tables" ; if none of the above are specified, nothing will be produced 
...
calc_auc <- function(all_data_split, sqrt=FALSE) {
  ...
  if (sqrt==TRUE){
    sum_table <- cbind(sum_table[,1:6], sqrt(sum_table[,7:24]))
  }
...
auc_summary <- function(ontogeny, summary_table) {
